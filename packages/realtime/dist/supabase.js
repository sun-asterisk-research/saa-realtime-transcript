 ...
 */
export async function isUserAuthorizedForSession(sessionCode, userEmail, userId) {
    const supabase = getSupabaseClient();
...
export async function getSessionByCode(code) {
    ...
    sessionCache.set(code, { session: data, timestamp: Date.now() });
    return data;
}
...
export async function broadcastStreaming(sessionCode, data) {
