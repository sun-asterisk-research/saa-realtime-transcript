import type { Token, ClientSession, TranslationConfig } from './types.js';
...
export class TranscriptHandler {
   ...
   */
  async processTokens(tokens: Token[]): Promise<void> {
   ...
   */
  private async handleNonFinalTokens(tokens: Token[]): Promise<void> {
    ...
    if (broadcastKey === this.lastBroadcastKey) return;
   ...
   */
  private async handleFinalTokens(tokens: Token[]): Promise<void> {
   ...
   */
  private needsTranslation(sourceLanguage?: string): boolean {
   ...
   */
  private getTargetLanguage(sourceLanguage?: string): string | undefined {
    const config = this.session.translationConfig;
